...
class TestAuth:
    ...
    async def test_auth_refresh_token(
        ...
        # if the token expires in less than 60 seconds it should get refreshed when
        # `async_get_access_token` is called
        auth_test_object.expect_login(
            TEST_EMAIL_ADDRESS, TEST_PASSWORD
        ...
        assert (
            ...
            == TOKEN_REFRESH_RETURN_VALUE["access_token"]
        )

        # shouldn't change the user id
        assert auth.user_id == TEST_USER_ID

    async def test_auth_refresh_token_refresh(
        self, auth_test_object: AuthTestObject, client_session: ClientSession
    ):
        """
        Each time the access token is refreshed, the next refresh call should use the
        refresh token from the previous call.
        """
        ...
        first_refresh_response = TOKEN_REFRESH_RETURN_VALUE | {"expires_in": "10"}
        second_refresh_response = TOKEN_REFRESH_RETURN_VALUE | {
            "access_token": "test-third-access-token"
        }
        auth_test_object.expect_token_refresh(
            LOGIN_RETURN_VALUE["refreshToken"]
        ).respond_with_json(first_refresh_response)
        auth_test_object.expect_token_refresh(
            TOKEN_REFRESH_RETURN_VALUE["refresh_token"]
        ).respond_with_json(second_refresh_response)
        ...
        assert (
            await auth.async_get_access_token()
            == first_refresh_response["access_token"]
        )
        assert (
            await auth.async_get_access_token()
            == second_refresh_response["access_token"]
        )
    ...
    async def test_unknown_error(
        ...
        """
        Test that a response with an invalid error reason is converted to an AuthenticationError
        """
