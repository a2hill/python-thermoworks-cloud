...
def parse_email_last_event(data: dict) -> Optional[EmailLastEvent]:
    """Parse emailLastEvent into an EmailLastEvent dataclass."""
    if not data or "fields" not in data:
        return None

    return map_firestore_fields(data["fields"], EmailLastEvent)


def parse_device_order_item(data: dict) -> Optional[DeviceOrderItem]:
    """Parse a device order item from Firestore data."""
    if not data or "fields" not in data:
        return None

    return map_firestore_fields(data["fields"], DeviceOrderItem)
...
def parse_device_order(data: dict) -> Dict[str, List[DeviceOrderItem]]:
    ...
    for account_id, devices in fields.items():
        ...
        for device in devices["arrayValue"]["values"]:
            ...
            item = parse_device_order_item(device["mapValue"])
            if item:
...
def parse_map_bool_values(fields: Dict, field_name: str) -> Optional[Dict[str, bool]]:
    """Parse a map of boolean values from Firestore data."""
    if (field_name not in fields or
            "mapValue" not in fields[field_name] or
            "fields" not in fields[field_name]["mapValue"]):
        return None

    result = {}
    for key, value in fields[field_name]["mapValue"]["fields"].items():
...
def document_to_user(document: dict) -> User:
    ...
    # Handle map values
    user.roles = parse_map_bool_values(fields, "roles")
    user.account_roles = parse_map_bool_values(fields, "accountRoles")
    user.system = parse_map_bool_values(fields, "system")
    user.notification_settings = parse_map_bool_values(
        fields, "notificationSettings")
    user.fcm_tokens = parse_map_bool_values(fields, "fcmTokens")
